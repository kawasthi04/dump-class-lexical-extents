...
void ASTFrontendAction::ExecuteAction() {
  ...
  // If -fdump-class-extents was specified, run the ClassExtentDumper now
  if (CI.getFrontendOpts().DumpClassExtents) {
    llvm::errs() << "Dumping class extents:\n";
    ClassExtentDumper Dumper(CI, llvm::errs());
    Dumper.HandleTranslationUnit(CI.getASTContext());
  }
}
